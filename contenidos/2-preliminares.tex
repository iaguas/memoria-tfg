...
\REV{funciones de agregación vs operadores de agregación ¿Qué es más correco para nombrarlos?}
...
Una  función $F:\unitinterval^n\rightarrow\unitinterval$ será una función OWA de dimensión $n$ si existe un vector de pesos $w=(w_{1},w_{2},\dots,w_{n})\in \unitinterval^{n}$ siempre que $\sum_{i}w_{i}=1$ de forma que
...
Por tanto para poder obtener un resultado adecuado, deberá utilizarse un vector de pesos $w$ que se adecue a las necesidades del problema. En este trabajo se emplea la versión `menor que la mitad' \REV{nombre} que tiene como vector de pesos a $w=(w_{1}\dots w_{i}, w_{i+1}\dots w_{n})$ sabiendo que $\abs{1, \dots, i}=\abs{i+1,\dots,n}$ de forma que $w_{j}=\frac{1}{2n}$ si $1\leq j\leq i$ y $w_{j}=0$ en el resto de los casos.\REV{buscar la versión buena, esto es sólo para los cardinales pares}.

Además, en la segunda parte del trabajo, se empea el OWA ``la mayoría de'' donde destaca con su vector de pesos a aquellos elementos mayores que los otros, esto es, a los que se encuentran en una posición superior a la mitad del vector ordenado.
...
\end{definition}
\begin{lemma}
La función de equivalencia de Dombi, $D$, cumplirá las siguientes propiedades:
\begin{enumerate}
